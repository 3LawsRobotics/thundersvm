/*
    Since the format of parameters are same as libsvm, we reference and modify
   command line parser source code of libsvm in this file.
...
void HelpInfo_svmtrain() {
  LOG(INFO)
      << "Usage (same as LibSVM): thundersvm [options] training_set_file "
         "[model_file]\n"
         "options:\n"
         ...
         "0.1)\n"
         "-m memory size: constrain the maximum memory size (MB) that "
         "thundersvm uses (default 8192)\n"
         ...
         "-o n: set the number of cpu cores to use, -1 for maximum(default "
         "-1)\n"
         "-q: quiet mode";
  exit(1);
}
...
void HelpInfo_svmpredict() {
  LOG(INFO)
      << "Usage: svm-predict [options] test_file model_file output_file\n"
         ...
         //                      "-b probability_estimates: whether to predict
         //                      probability estimates, 0 or 1 (default 0); for
...
void CMDParser::parse_command_line(int argc, char **argv) {
  ...
  if (bin_name == "thundersvm-train" || bin_name == "thundersvm-train.exe") {
    ...
    for (i = 1; i < argc; i++) {
      ...
      case 's':
        ...
        break;
      case 't':
        param_cmd.kernel_type =
            static_cast<SvmParam::KERNEL_TYPE>(atoi(argv[i]));
        break;
      case 'd':
      ...
      case 'n':
        ...
        break;
      case 'm':
        param_cmd.max_mem_size = static_cast<size_t>(max(atoi(argv[i]), 0))
                                 << 20; // MB to Byte
        break;
      case 'c':
      ...
      case 'w':
        ++param_cmd.nr_weight;
        param_cmd.weight_label = (int *)realloc(
            param_cmd.weight_label, sizeof(int) * param_cmd.nr_weight);
        param_cmd.weight = (float_type *)realloc(
            param_cmd.weight, sizeof(double) * param_cmd.nr_weight);
    ...
    if (i < argc - 1)
      ...
      model_file_name += ".model";
             ...
             bin_name == "thundersvm-predict.exe") {
    for (i = 1; i < argc; i++) {
      if (argv[i][0] != '-')
        break;
      i++;
  ...
  el::Loggers::reconfigureAllLoggers(el::ConfigurationType::Enabled,
                                     quiet ? "false" : "true");
}
...
bool CMDParser::check_parameter() {
  ...
  if (svm_type == SvmParam::C_SVC || svm_type == SvmParam::EPSILON_SVR ||
      svm_type == SvmParam::NU_SVR)
    if (param_cmd.C <= 0) {
      LOG(ERROR) << "C <= 0";
      return false;
    }
  if (svm_type == SvmParam::NU_SVC || svm_type == SvmParam::ONE_CLASS ||
      svm_type == SvmParam::NU_SVR)
  ...
  if (param_cmd.probability == 1 &&
      (svm_type == SvmParam::ONE_CLASS || svm_type == SvmParam::EPSILON_SVR ||
       svm_type == SvmParam::NU_SVR)) {
