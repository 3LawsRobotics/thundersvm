...
DataSet::DataSet(const DataSet::node2d &instances, int n_features,
                 const vector<float_type> &y)
    : instances_(instances), n_features_(n_features), y_(y),
...
#pragma omp parallel num_threads(nthread)
    ...
    for (int i = 0; i < nthread; i++) {
      ...
      this->instances_.insert(instances_.end(), instances_thread[i].begin(),
                              instances_thread[i].end());
    }
  }
  free(buffer);
  LOG(INFO) << "#instances = " << this->n_instances()
            << ", #features = " << this->n_features();
}
...
void DataSet::load_from_python(float *y, char **x, int len) {
  ...
  for (int i = 0; i < len; i++) {
    ...
    string tuple;
    while (ss >> tuple) {
      CHECK_EQ(sscanf(tuple.c_str(), "%d:%f", &ind, &v), 2)
          << "read error, using [index]:[value] format";
...
void DataSet::load_from_sparse(int row_size, float *val, int *row_ptr,
                               int *col_ptr, float *label) {
  y_.clear();
  instances_.clear();
  total_count_ = 0;
  n_features_ = 0;
...
void DataSet::load_from_dense(int row_size, int features, float *data,
                              float *label) {
  y_.clear();
  instances_.clear();
  total_count_ = 0;
...
const vector<int> &
DataSet::count() const { // return the number of instances of each class
  return count_;
}

const vector<int> &DataSet::start() const { return start_; }

size_t DataSet::n_classes() const { return start_.size(); }

const vector<int> &DataSet::label() const { return label_; }
...
void DataSet::group_classes(bool classification) {
  if (classification) {
    ...
    for (int i = 0; i < y_.size(); ++i) {
      ...
      if (j == label_.size()) {
        // real to int conversion is safe, because group_classes only used in
        // classification
    ...
    perm_ =
        vector<int>(y_.size()); // index of each instance in the original array
    for (int i = 0; i < y_.size(); ++i) {
...
size_t DataSet::n_features() const { return n_features_; }
...
DataSet::instances(int y_i) const { // return instances of a given class
  int si = start_[y_i];
  int ci = count_[y_i];
...
const DataSet::node2d
DataSet::instances(int y_i, int y_j) const { // return instances of two classes
...
const vector<int> DataSet::original_index()
    const { // index of each instance in the original array
  return perm_;
}

const vector<int> DataSet::original_index(int y_i)
    const { // index of each instance in the original array for one class
  return vector<int>(perm_.begin() + start_[y_i],
                     perm_.begin() + start_[y_i] + count_[y_i]);
}

const vector<int> DataSet::original_index(int y_i, int y_j)
    const { // index of each instance in the original array for two class
...
const vector<float_type> &DataSet::y() const { return y_; }

const bool DataSet::is_zero_based() const { return zero_based; }
