...
//            vector<float_type> test_predict =
//            model->cross_validation(train_dataset, parser.param_cmd,
//            parser.nr_fold); int dataset_size = test_predict.size() / 2;
//    	   test_y.insert(test_y.end(), test_predict.begin(),
//    test_predict.begin() + dataset_size); predict_y.insert(predict_y.end(),
//    test_predict.begin() + dataset_size, test_predict.end());
//        } else {
...
//
//        if (metric) {
//            LOG(INFO) << metric->name() << " = " << metric->score(predict_y,
//            predict_dataset.y());
//        }
//    }

int *train_R(char **dataset, int *kernel, int *svm_type, int *degree,
             char **gamma, double *coef0, double *nu, double *cost,
             double *epsilon, double *tol, int *probability,
             char **class_weight, int *weight_length, int *n_fold, int *verbose,
             int *max_iter, int *n_cores, char **model_file) {
  ...
  if ((strcmp(gamma[0], "auto") == 0) &&
      (param_cmd.kernel_type != SvmParam::LINEAR)) {
    printf("in auto\n");
  ...
  if (strcmp(class_weight[0], "None")) {
    ...
    param_cmd.weight =
        (float_type *)malloc(param_cmd.nr_weight * sizeof(float_type));
  ...
  if (do_cross_validation) {
    ...
    if (metric) {
      std::cout << metric->name() << " = "
                << metric->score(predict_y, train_dataset.y()) << std::endl;
    }
    delete metric;
  }
  delete model;
...
void predict_R(char **test_file, char **model_file, char **out_file) {
  ...
  if (metric) {
    LOG(INFO) << metric->name() << " = "
              << metric->score(predict_y, predict_dataset.y());
  }
  delete model;
  delete metric;
}
