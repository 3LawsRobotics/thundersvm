...
void init_model_param(char *kernel_type, int *degree, float *gamma,
                      float *coef0, int *probability, SvmModel *model) {
  model->get_param(kernel_type, degree, gamma, coef0, probability);
}

void sparse_model_scikit(int row_size, float *val, int *row_ptr, int *col_ptr,
                         ...
                         int *n_classes, int *succeed, SvmModel *model) {
#ifdef USE_CUDA
...
int sparse_predict(int row_size, float *val, int *row_ptr, int *col_ptr,
                   SvmModel *model, float *predict_label, int verbose) {
  if (verbose)
    el::Loggers::reconfigureAllLoggers(el::ConfigurationType::Enabled, "true");
  else
    el::Loggers::reconfigureAllLoggers(el::ConfigurationType::Enabled, "false");
  DataSet predict_dataset;
  predict_dataset.load_from_sparse(row_size, val, row_ptr, col_ptr,
                                   (float *)NULL);
...
void dense_model_scikit(int row_size, int features, float *data, float *label,
                        ...
                        int max_iter, int n_cores, int max_mem_size, int gpu_id,
...
int dense_predict(int row_size, int features, float *data, SvmModel *model,
                  float *predict_label, int verbose) {
  if (verbose)
    el::Loggers::reconfigureAllLoggers(el::ConfigurationType::Enabled, "true");
  else
    el::Loggers::reconfigureAllLoggers(el::ConfigurationType::Enabled, "false");
...
int n_sv(SvmModel *model) { return model->total_sv(); }
...
void get_sv(int *row, int *col, float *data, int *data_size, int *sv_indices,
            SvmModel *model) {
...
void get_linear_coef(float *linear_coef, int n_binary_model, int n_feature,
                     SvmModel *model) {
...
void sparse_decision(int row_size, float *val, int *row_ptr, int *col_ptr,
                     SvmModel *model, int value_size, float *dec_value) {
  DataSet predict_dataset;
  predict_dataset.load_from_sparse(row_size, val, row_ptr, col_ptr,
                                   (float *)NULL);
...
void dense_decision(int row_size, int features, float *data, SvmModel *model,
                    int value_size, float *dec_value) {
  DataSet predict_dataset;
  predict_dataset.load_from_dense(row_size, features, data, (float *)NULL);
  model->predict(predict_dataset.instances(), -1);
  // SyncArray<float_type> dec_value_array(value_size);
